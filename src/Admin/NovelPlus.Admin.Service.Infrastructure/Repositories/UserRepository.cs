    ...
    : RepositoryBase<UserEntity>(logger, options), IUserRepository
{
    ...
    public Task<int> SaveAsync(UserEntity entity) => SaveAsyncInternal(entity);
    public Task<int> UpdateAsync(UserEntity entity) => UpdateAsyncInternal(entity);
    ...
    {
        var sql = "SELECT DATE_FORMAT(create_time, '%Y-%m-%d') AS StaDate, COUNT(1) AS StaCount FROM User WHERE create_time >= @MinDate GROUP BY DATE_FORMAT(create_time, '%Y-%m-%d') ORDER BY StaDate";
